...
1. 再复杂的业务系统中，当一个sql语句需要锁表，会导致暂时不能读取，那么就会很影响运行的服务，使用主从复制，让主库负责写，从库负责读；
2. 数据备份
3. 随着业务的扩大，i/o访问的频率过高，单机无法满足，此时做多库存储，降级i/o访问的频率

## 复制原理

1. 从库会生成两个线程，一个i/o线程，一个sql线程；
2. i/o线程会去请求主库的binlog，并将得到的binlog写到本地的中继日志中；
3. 主库会生成一个log dump线程，用来给从库传送binlog;
4. sql线程用来读取中继日志，解析成sql执行；

![](.\image\主从原理.png)

## mysql主从同步延时分析

mysql的主从复制都是单线程操作，主库对所有的ddl和dml的操作写入binlog日志，由于binlog 是顺序写，所以效率很高，slave的i/o线程也是顺序写入中继日志，**`但是sql线程从中继日志中读取到操作后，写入操作是随机的，所以时间成本很高`;另外sql线程也是单线程，如果主库的并发较高，产生的日志超过了从的sql线程处理的速度，就产生了延时；如果从库同步数据的时候，跟别的线程发生锁抢占的情况，也是会发生延时**

### 如何察看同步延迟状态

可以通过从服务器来查看show slave status
...
其中seconds_behind_master是最重要的参数，表示当前备库延迟了多长时间

- 解决方法：

  1. 物理解决，添加机器，不同业务数据库放到不同的机器上，分散压力；使用更强的硬件设备；
  ...
  2. 从根本上解决延时问题：并行复制
     ...
     > ## **`注：执行同一行的两个事务，必须分发到同一个worker中；同一个事务多个执行操作不能被拆开必须放到一个worker中`**
